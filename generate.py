...
THEMES = {
    ...
    "zcodes": dict(
        # 5-stop contribution ramp: empty..brightest — same palette as website + streak cards
        levels=["#0d1117", "#3a0e24", "#7a0c2e", "#ff4d6d", "#ff9db3"],
        snake="#ff4d6d", head="#ff9db3",
        ramp=[(1, "#ff9db3"), (4, "#ff4d6d"), (10, "#7a0c2e"), (21, "#3a0e24")],
        text="#eaeaea", frame_bg="#0d1117", frame_border="#ff4d6d",
    ),
}
...
def render(grid, counts, months, route, eats, growth, theme, opts):
    ...
    for c in range(ncols):
        for r in range(len(grid[c])):
            ...
            cls = f"c{c}_{r}"
            body.append(f'<rect class="{cls}" x="{x}" y="{y}" width="{CELL}" height="{CELL}" rx="5.5" fill="{base}"/>')
            if not ivs:
                continue
            stops = [(0.0, base)]
            for a, b in ivs:
    ...
    for i in range(5):
        body.append(f'<rect x="{lx+i*(CELL+3)}" y="{ly}" width="{CELL}" height="{CELL}" rx="5.5" fill="{t["levels"][i]}"/>')
    ...
    # --- Snake extras: head with mouth/eyes, pointy tail, body growth already 2× ---
    eat_steps = {s for s,_ in eats}
    # Head path: for each route step, head position; tail = oldest body cell
    head_moves = []
    tail_moves = []
    for s in range(n):
        hx, hy = xy(route[s][0], route[s][1])
        head_moves.append((pct(s), hx, hy))
        # tail follows with current snake length
        ti = max(0, s - length_at(s) + 1)
        tx, ty = xy(route[ti][0], route[ti][1])
        tail_moves.append((pct(s), tx, ty))
    # Head CSS: translate the whole head group; mouth open on eating frames
    def build_move_css(name, moves):
        steps = [f"{p}% {{ transform: translate({x}px, {y}px); }}" for p,_x,_y in [(m[0],m[1],m[2]) for m in moves]]
        # add final hold to pause
        steps.append(f"100% {{ transform: translate({moves[-1][1]}px, {moves[-1][2]}px); }}")
        return f"@keyframes {name} {{ {' '.join(steps)} }} .{name} {{ animation:{name} {dur:.1f}s linear infinite; }}"

    css.append(build_move_css("headMove", head_moves))
    css.append(build_move_css("tailMove", tail_moves))
    # Mouth open/close: eating frames get open mouth (wider polygon), else closed
    # We'll toggle via separate animation on the mouth polygon's points (via opacity of two variants)
    mouth_css = []
    for s in range(n):
        a = pct(s)
        b = pct(s+1) if s+1 < n else 100.0
        open_here = s in eat_steps or (s % 8 < 2)  # eat = open, else brief open every 8 steps for life
        # two polygons: mouthClosed (single line) and mouthOpen (wedge) — toggle opacity
        mouth_css.append((a, open_here))
    # Build mouth keyframes (opacity swap)
    oc_frames = []
    oo_frames = []
    for s in range(n):
        p = pct(s)
        is_open = s in eat_steps or (s % 8 < 2)
        oc_frames.append(f"{p}% {{ opacity:{0 if is_open else 1}; }}")
        oo_frames.append(f"{p}% {{ opacity:{1 if is_open else 0}; }}")
    oc_frames.append("100% { opacity:1; }")
    oo_frames.append("100% { opacity:0; }")
    css.append(f"@keyframes mouthClosed {{ {' '.join(oc_frames)} }} .mouthClosed {{ animation:mouthClosed {dur:.1f}s steps(1,end) infinite; }}")
    css.append(f"@keyframes mouthOpen {{ {' '.join(oo_frames)} }} .mouthOpen {{ animation:mouthOpen {dur:.1f}s steps(1,end) infinite; }}")

    # Head sprite: circle body + eyes + two mouth variants
    # Positioned at 0,0; the outer <g> translates via headMove
    head_sprite = (
        f'<g class="headMove" style="transform: translate({head_moves[0][1]}px, {head_moves[0][2]}px);">'
        f'<rect x="0" y="0" width="{CELL}" height="{CELL}" rx="5.5" fill="{t["head"]}" stroke="#0d1117" stroke-width="1"/>'
        f'<circle cx="3" cy="3.5" r="1.2" fill="#0d1117"/><circle cx="8" cy="3.5" r="1.2" fill="#0d1117"/>'
        f'<polygon class="mouthClosed" points="3,8 8,8 5.5,9.5" fill="#0d1117"/>'
        f'<polygon class="mouthOpen" points="2,7 9,7 5.5,11" fill="#0d1117"/>'
        f'</g>'
    )
    # Pointy tail: triangle pointing opposite of movement direction (simple upward point; rotates via direction not yet — keep generic)
    tail_sprite = (
        f'<g class="tailMove" style="transform: translate({tail_moves[0][1]}px, {tail_moves[0][2]}px);">'
        f'<polygon points="5.5,1 1,10 10,10" fill="{t["snake"]}" stroke="#0d1117" stroke-width="0.8"/>'
        f'</g>'
    )
    body.append(head_sprite)
    body.append(tail_sprite)

    w = MX * 2 + ncols * PITCH - GAP
    h = MTOP + 7 * PITCH - GAP + MBOT
    # About-Me style card wrapper — matches portfolio card (always, regardless of --frame)
    about_glow = '<defs><filter id="cardGlow" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dx="0" dy="0" stdDeviation="10" flood-color="#ff4d6d" flood-opacity="0.12"/></filter></defs>'
    card_open = f'<rect x="2" y="2" width="{w-4}" height="{h-4}" rx="16" fill="#0d1117" stroke="#ff4d6d" stroke-opacity="0.25" filter="url(#cardGlow)"/>'
    frame_open = about_glow + card_open
    frame_close = ""
    if opts.frame:
        # extra inner frame not needed now; keep for optional window chrome
        frame_close = f'<rect x="1" y="1" width="{w-2}" height="{h-2}" rx="12" fill="none" stroke="{t["frame_border"]}" opacity="0.3"/>'
    mono = "JetBrains Mono,ui-monospace,'SF Mono',Menlo,Consolas,monospace"
    return f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {w} {h}">
  <style>
    .lab {{ font-family:{mono}; font-size:9.5px; fill:{t['text']}; }}
    .cnt {{ font-family:{mono}; font-size:9.5px; fill:#ff9db3; font-weight:600; }}
